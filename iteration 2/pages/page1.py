...
with st.expander("Questions."):
    ...
    question2 = st.radio(
        "Using the toilet in a dry spot. John and his parents have one toilet, located downstairs. Do you think John and his parents are able to use their toilet in a dry spot?",
        ('Yes', 'No'))
    
    question3 = st.radio(
        "Having at least three litres of bottled water per person to survive for three days. John and his parents don't have any bottled drinks at home. His mother quickly checked if the tap in their upstairs bathroom works, but that is not the case. Do you think John and his parents are able to have enough drinks left to survive for three days?",
        ('Yes', 'No'))
    
    question4 = st.radio(
        "Having food for three days. All their food is located in their basement, which is probably flooded too. Do you think John and his parents are able to have enough food left to survive for three days?",
        ('Yes', 'No'))
    
    question5 = st.radio(
        "Having a sufficient amount of clothing and blankets to stay warm. The elecricity in John's house doesn't work anymore. John's father mentioned that they have enough blankets left upstairs. Do you think John and his parents have a sufficient amount of clothing and blankets to stay warm?",
        ('Yes', 'No'))
    
    question6 = st.radio(
        "John's phone is almost empty. The phones of his parents are still fully charged. Do you think John and his parents are able to stay for a couple of days in touch with their friends/family/relatives?",
        ('Yes', 'No'))
    ...
    question8 = st.radio(
        "Having a sufficient amount of medicine at home for three days. John's mother uses blood thinners. They wanted to get some new ones tomorrow, as she only has enough left for one day. Do you think John has a sufficient amount of medicine at home her mother for three days → Yes or no?",
        ('Yes', 'No'))
...
with st.expander("Housing questions."):
    inputArea = st.slider('Amount of square metres (m²):', 1, 105, 52)

    inputBedrooms = st.slider('Minimum amount of bedrooms for your family', 1, 5, 3)

    inputPropertyType = st.radio("Would an apartment or house suit your situation the best?", ('apartment', 'house'))

    inputRooms = st.slider('Preferred amount of rooms besides bedrooms to suit family:', 1, 6, 3)

    inputYear = st.slider('Would you rather live in an old or newer apartment/house?', 1900, 2017, 1955)


    if st.button("Find my house"):
        user_input = {}
...
with col1:
   ...
   st.markdown(selected_rows.iloc[0])

with col2:
   st.header("Recommended house 2:")
   st.markdown(selected_rows.iloc[1])

with col3:
   st.header("Recommended house 3:")
   st.markdown(selected_rows.iloc[2])



# display the selected rows in the terminal
print(selected_rows.iloc)
